...
class FilterManager:
    ...
    def _apply_sepia(frame: np.ndarray) -> np.ndarray:
        """Apply sepia tone effect (classic warm brown)."""
    ...
    def _apply_silver_screen(frame: np.ndarray) -> np.ndarray:
        """Apply silver screen effect (1930s-50s high contrast B&W)."""
    ...
    def _apply_70s_film(frame: np.ndarray) -> np.ndarray:
        """Apply 70s film effect (warm Kodak Portra look with grain)."""
    ...
    def _apply_vhs_simple(frame: np.ndarray) -> np.ndarray:
        """Apply VHS effect (simple RGB shift - fast)."""
        b, g, r = cv2.split(frame)
        r_shifted = np.roll(r, 2, axis=1)
        b_shifted = np.roll(b, -2, axis=1)
        result = cv2.merge([b_shifted, g, r_shifted])
        result = np.clip(result.astype(np.float32) * 0.95, 0, 255).astype(np.uint8)
        return result

    @staticmethod
    def _apply_vhs_complex(frame: np.ndarray) -> np.ndarray:
        """Apply VHS effect (complex chromatic aberration - detailed)."""
        b, g, r = cv2.split(frame)
        r_blur = cv2.GaussianBlur(r, (3, 3), 0)
        b_blur = cv2.GaussianBlur(b, (3, 3), 0)
        r_shifted = np.roll(r_blur, 3, axis=1)
        b_shifted = np.roll(b_blur, -3, axis=1)
        result = cv2.merge([b_shifted, g, r_shifted])
        gray = cv2.cvtColor(result, cv2.COLOR_BGR2GRAY)
        gray_3ch = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
        result = cv2.addWeighted(result, 0.85, gray_3ch, 0.15, 0)
    ...
    def _apply_super8(frame: np.ndarray) -> np.ndarray:
        """Apply Super 8 effect (warm vignette + grain)."""
        h, w = frame.shape[:2]
        center_x, center_y = w // 2, h // 2
        Y, X = np.ogrid[:h, :w]
        dist = np.sqrt((X - center_x) ** 2 + (Y - center_y) ** 2)
        max_dist = np.sqrt(center_x ** 2 + center_y ** 2)
        vignette = np.clip(1 - (dist / max_dist) * 0.8, 0, 1)
        ...
        noise = np.random.normal(0, 20, frame.shape).astype(np.float32)
        result = np.clip(result + noise, 0, 255)
        return result.astype(np.uint8)